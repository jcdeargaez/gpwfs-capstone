...
let create ownerName (accountId: Guid option) =
    ...
    { Owner = { Name = ownerName }; AccountId = accountId'; Balance = 0M }
    |> classifyAccount
...
let auditAs operation audit amount account =
    let auditSuccessfulOperation account' =
        account'
        |> iter (fun a ->
            let tx = { Timestamp = DateTime.UtcNow; Operation = operation; Amount = amount }
...
let loadAccount (ownerName, accountId, transactions) =
    ...
    let initialAccount = create ownerName accountId
